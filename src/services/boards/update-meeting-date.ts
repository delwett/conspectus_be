...
export default async function updateMeetingDate(newDate: Date): Promise<Board> {
  ...
  const currentDate = new Date(currentBoard.meetingDate)

  if (isPast(currentDate)) throw new ValidationError('Meeting date is prohibited to change during running meeting')
